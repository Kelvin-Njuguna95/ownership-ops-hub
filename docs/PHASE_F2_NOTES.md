...
| Name | Value | Source |
|---|---|---|
| `AIRTABLE_PAT` | Personal access token with read access to base `REDACTED_BASE_ID` (table `relations_support`) | Airtable → Account → Developer hub → Personal access tokens. Scopes: `data.records:read`. Workspaces: the CLIENT_A base. |
