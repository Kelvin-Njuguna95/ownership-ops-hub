...
- **New Companies hero = companies, not records (by design).** The `.kpi.hero` big number on the New Companies page (`PAGE_RENDERERS.new_companies`) is `uniqueCos` — the distinct `add_new_company` count (218 on 2026-06-04) — and the record count `total` (`recs.length`, 485) lives in the `.nc-sub` "Across N tagged records" line. This is the intended order per Kelvin's 2026-06-04 request; do NOT "fix" it back to leading with the record count. The other three cards (Still open / Cleared to Valid / Quality gaps) are deliberately record-based.
